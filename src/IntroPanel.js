...
class IntroPanel extends React.Component {
  ...
  getHeader() {
    return (
      <div className="header">
        <div className="info">
          <TriggerIcon iconType={ICON_TYPE.INFO_ICON} />
        </div>
  ...
  getNav() {
    const sections = SECTIONS.map((s, i) => {
      ...
      return (
        <p className={classes} onClick={this.setActiveSection.bind(this, i)} key={s.name}>
          {s.name}
        </p>
      );
    });

    return <div className="nav">{sections}</div>;
  }
  ...
  getCitationButton = () => {
    ...
    if (SECTIONS[activeSectionIdx].citationButton) {
      if (SECTIONS[activeSectionIdx].citationButton.use === true) {
        const citation =
          SECTIONS[activeSectionIdx][SECTIONS[activeSectionIdx].citationButton.copies];
    ...
    return (
      <div className={classes}>
        ...
        <div className="intro-panel">
          ...
          <div className="content-nav">{this.getNextPrev()}</div>
        </div>
      </div>
    );
  }
